...
export class LookupTable {
    ...
    * values.
   */
   constructor(minimum: number, maximum: number, tableSize: number, public transformFunc: ITransformFunc) {
...
export enum LevelScale { Linear, ProbabilityDensity }
...
export class Algorithms {
  ...
  */
   ...
   */
   static getZoomViewportPercent(devWidth: number, devHeight: number, zoomTransform: { x: number, y: number, k: number }) {
    ...
    */
   static generateLinearLookup(histogram: Float32Array | Uint32Array | number[], percent: number): Float32Array {
   ...
   /**
      ...
      * @param percent 
      */
   static generateLevels(histogram: Float32Array | Uint32Array | number[], scale: LevelScale, percent: number): Float32Array {
    ...
    * @param dataRange 
    */
   static generateDensity(xy: Float32Array, histDim: number,
      dataRange: { xmin: number, xmax: number, ymin: number, ymax: number, xScale: ScaleType, yScale: ScaleType }, smooth: boolean = true) {
      ...
      xform.coordinateSystemTransform(dataRange.xmin, dataRange.xmax, dataRange.ymin, dataRange.ymax, 0, histDim - 1, 0, histDim - 1);
    ...
    */
   static generateFcsDensity(xy: Float32Array, histDim: number,
      dataRange: {
         xmin: number, xmax: number,
         ymin: number, ymax: number,
         xScale: ScaleType, yScale: ScaleType, maxRange: number
      ...
      let xform = new Matrix();
      xform.coordinateSystemTransform(scaledRange.xmin, scaledRange.xmax, scaledRange.ymin, scaledRange.ymax, 0, histDim - 1, 0, histDim - 1);

      // get the correct scale function: identity/log/glog
      let xMult = (val: number) => { return val; };
      ...
      let yMult = (val: number) => { return val; };
   ...
   static genGaussianData(numPts: number, viewport: { xmin: number, xmax: number, ymin: number, ymax: number }) {
      let m = new Matrix();
      m.coordinateSystemTransform(-3, 3, -3, 3, viewport.xmin, viewport.xmax, viewport.ymin, viewport.ymax);
      let data: number[] = [];
...
export class DropVolumeAlg {
   ...
   static getNozzleVolumeTable() {
      return [
         { nozzle: "Nozzle70", volume: 1.12787356E-06 },    // These nozzle strings are predetermined
         { nozzle: "Nozzle85", volume: 2.39361702e-06 },
         { nozzle: "Nozzle100", volume: 3.47222222e-06 },   // currently only one type no a/b for 100 and 130
         { nozzle: "Nozzle130", volume: 5.29935275e-06 },
      ];
   }
   ...
   */
   static calculateCellsForNozzleAndVolume(nozzle: string, sampleVolume: number, yieldMask: number = 0) {
      let dropVolume = DropVolumeAlg.getDropVolumeForNozzle(nozzle);
      let numberCells = DropVolumeAlg.calculateCellsFromVolume(sampleVolume, dropVolume, yieldMask);
    ...
    * @returns the number of cells
    */
   static calculateCellsFromVolume(sampleVolume: number, dropVolume: number, yieldMask: number = 0) {
      let numberCells = 0;
      if (dropVolume > 0 && yieldMask >= 0 && yieldMask <= 32) {
         numberCells = (sampleVolume / dropVolume) / (1 + yieldMask / 32.0);
      }

      return (numberCells + .5) | 0;
   }
...
export class VertexSegmenter {
   ...
   static convertToSubsegments(srcPts: IPoint[]): IPoint[] {
      ...
      matrix.coordinateSystemTransform(bounds.xMin, bounds.xMax, bounds.yMin, bounds.yMax, 0, dim - 1, 0, dim - 1);
      ...
      matrix = new Matrix();
      matrix.coordinateSystemTransform(0, 255, 0, 255, bounds.xMin, bounds.xMax, bounds.yMin, bounds.yMax);
