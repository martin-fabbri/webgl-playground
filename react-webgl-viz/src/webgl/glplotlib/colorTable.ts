...
export class ColorTable {
    ...
    static getAllColorTables() {
        ...
        return {
            numberTables: colorTables.length,
            tableLength: colorTables[0].length,
            colorTables: new Uint8Array(table)
        };
    }

    private static buildColorTable(func: (val: number) => string, reverseTable: boolean = false) {
        const TABLELEN = 256;
        let colorScale = d3.scaleSequential(func).domain([0, TABLELEN]);
        ...
        if (!this.cssPalettes) {
            ...
            this.getRgbPalettes().forEach(palette => {
                ...
                for (let i = 0; i < palette.length; i += 3) {
                    let color = `#${this.pad(palette[i].toString(16))}${this.pad(
                        palette[i + 1].toString(16)
                    )}${this.pad(palette[i + 2].toString(16))}`;
